...
"""
def openFile(infile):
    BG_THRESHOLD = 10   # Color of the background. Change to a smaller value if the program is being over-aggressive with the background detection

    image = cv2.imread(infile, 1);
    if image == "":
        print "can't read avatars!"
    ...
    avatar_only = new_img[y:y+h, x:x+w]
...
def main():
    infile = "./users/Cheetah/avatar1/frames000001.jpg" 
    outfile = "outputAvatar.png" 
    a = openFile(infile)
    print a.shape[1]
